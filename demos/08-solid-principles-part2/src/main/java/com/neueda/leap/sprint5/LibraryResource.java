...
// LibraryResource("001") is a compile error) - it exists only to be extended. And
// because calculateLateFee() has NO body here, every subclass is REQUIRED to supply
// its own, or that subclass must also be declared abstract. A class that forgets
// to override it simply won't compile any more - the exact bug from Step 1 is now
// impossible, enforced by the compiler, not by someone remembering.
public abstract class LibraryResource implements Feeable {
    ...
    // No body - subclasses MUST provide one. Compare to ConcreteInheritance's
    // version, which had a real (and dangerously reusable) default.
    @Override
    public abstract double calculateFee(double loanDuration);
}
